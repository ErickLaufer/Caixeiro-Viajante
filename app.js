...
function fitness(route, distMatrix) {
    const totalDistance = route.reduce((sum, city, i) => sum + (distMatrix[city][route[i + 1] || route[0]]), 0);
...
function selection(population, distMatrix) {
    ...
    return tournament.reduce((best, contender) => (fitness(contender, distMatrix) > fitness(best, distMatrix) ? contender : best));
}
...
function geneticAlgorithm(filename, popSize = 100, generations = 500, mutationRate = 0.1) {
    ...
    for (let gen = 0; gen < generations; gen++) {
        ...
        if (gen % 50 === 0) {
            console.log(`Geração ${gen}, melhor distância: ${1 / fitness(population[0], distMatrix)}`);
        }
    }
    ...
    const bestDistance = 1 / fitness(bestRoute, distMatrix);
...
console.log("Distância total:", bestDistance);
